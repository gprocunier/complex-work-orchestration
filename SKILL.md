...
- Every native operative worker requires `scripts/supervise_native_worker.py`. After trusted no-tools attestation, create and arm supervision before sending the task; the dispatch prompt renders only against that armed state (`render --supervision-state`), and returns validate only against finalized receipts (`validate-return --supervision-state`).
- Session telemetry must pass the trusted session-file invariant (regular file, supervising-user ownership, no group/world write); the supervisor and session checker fail closed on violations.
...
- A stale arm window, missing dispatch receipt, wrong control-turn ID, or late poll is control loss: issue native interrupt, close, record receipts, and never auto-launch a salvage worker. Do not use a long blocking wait as the monitor; measure spurious-vs-substantive control losses with `scripts/render_control_effectiveness.py` and tune lag tolerances from that evidence.
