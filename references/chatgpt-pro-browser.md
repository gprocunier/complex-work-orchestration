...
Use a compact plan bundle in a repository-local ignored directory:
...
python3 scripts/chatgpt_browser_review.py \
  ...
  --dry-run \
  --json \
  > "$WORK_PACKET_DIR/master-plan-review-dry-run.json"
```
...
python3 scripts/normalize_contractor_return.py \
  ...
  --dispatch-id "$DISPATCH_ID" \
  --packet-sha256 "$PACKET_SHA256" \
  --file "$WORK_PACKET_DIR/master-plan-review-return.md" \
  --output "$WORK_PACKET_DIR/master-plan-review-return-bundle.json"

python3 scripts/evaluate_return.py \
  --bead <id> \
  --file "$WORK_PACKET_DIR/master-plan-review-return.md" \
  --workspace-mutation-report mutation-report.json \
  > "$WORK_PACKET_DIR/master-plan-review-evaluation.json"
```
